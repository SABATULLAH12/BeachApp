...
namespace coke_beach_reportGenerator_api.Services
{
    public class ReportGeneratorBusiness : IReportGeneratorBusiness
    {
        ...
        public List<SampleSizeModel> CheckSampleSize(LeftPanelRequest leftPanel)
        {
            ...
            {
                /*if(el.SelectionType== "Benchmark")
                {
                    el.DetailedText = leftPanel.BenchmarkMenu.detailedText+;
                }
                else
                {
                     var rowTemp = comparisonTable.AsEnumerable().Where(a => a.Field<int>("SelectionID") == el.SelectionID).FirstOrDefault();
                    string categoryIDText = rowTemp.Field<int>("CategoryID").ToString();
                    string lowLevelCategoryText = rowTemp.Field<object>("LowLevelCategoryID") == null ? "" : rowTemp.Field<int>("LowLevelCategoryID").ToString();
                    string trademarkIDText = rowTemp.Field<object>("TrademarkID") == null ? "" : rowTemp.Field<int>("TrademarkID").ToString();
                    string brandIDText = rowTemp.Field<object>("BrandID") == null ? "" : rowTemp.Field<int>("BrandID").ToString();

                    string parentMapText = brandIDText + trademarkIDText;
                    parentMapText = parentMapText.Length == 0 ? lowLevelCategoryText : lowLevelCategoryText+","+parentMapText;
                    parentMapText = parentMapText.Length == 0 ? categoryIDText : categoryIDText + "," + parentMapText;

                    el.DetailedText = leftPanel.ComparisonMenu.Find(a => a.parentMap == parentMapText && a.consumptionId == el.ConsumptionId).detailedText;
