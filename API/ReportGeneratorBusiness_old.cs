...
namespace coke_beach_reportGenerator_api.Services
{
    public class ReportGeneratorBusiness : IReportGeneratorBusiness
    {
        ...
        public ReportGeneratorBusiness(IReportGeneratorService reportGeneratorService)
        {
        ...
        public MemoryStream FormatData(LeftPanelRequest leftPanel)
        {
            ...
            using (Presentation pres = new Presentation(stream))
            {
                ...
                GenerateOutputPPT(leftPanel.ComparisonMenu.Count, pres, pptDataSlide9, 10, leftPanel, imagesList); // 11 & 12
                ...
                pres.Slides.Remove(pres.Slides[11]);
                stream.Dispose();
                foreach (var image in imagesList)
                {
                    if (image.Image != null)
        ...
        {
            BlobClient blobClient = new BlobClient(Environment.GetEnvironmentVariable("AzureWebJobsStorage"), Environment.GetEnvironmentVariable("InputContainerName"), $"license.xml");
            var stream = new MemoryStream();
        ...
        private void GenerateOutputPPT(int numberOfCharts, Presentation pres, List<PPTBindingData> pptData, int SlideNumber, LeftPanelRequest leftPanel, List<ImagesList> imagesList)
        {
            if (SlideNumber == 7) // for slide 8, 9 & 14:
            {
                ...
                ISlide cur_Slide14 = pres.Slides[13]; // slide 14 
                ...
                BindSlideFourteenData(pptData, cur_Slide14, numberOfCharts, 13, imagesList); // slide 14
            }
            ...
            {
                ISlide cur_Slide11 = pres.Slides[SlideNumber]; // slide 11
                ISlide cur_Slide12 = pres.Slides[SlideNumber + 1]; // slide 12
        ...
        private void EditSelectionText(Presentation pres, LeftPanelRequest leftPanel)
        {
            ...
            string geoSelectionText = $"Geography: {leftPanel.GeographyMenu.text}";
            tempShape4.TextFrame.Text = $"{leftPanel.GeographyMenu.text}";

            // Change Filter Selection Text
            var filterTextArray = (from r in leftPanel.FilterMenu.AsEnumerable()
                                   select new List<string> { "BEVERAGEFILTER" }.Contains(r.type.ToUpper()) && r.levelId>=4 ? (r.detailedText + ": " + r.consumptionName) : r.text
                              ).ToArray();
        ...
        private void BindSlideData(List<PPTBindingData> pptData, ISlide cur_Slide, int numberOfCharts, List<string> metricType, int SlideNumber, List<ImagesList> imagesLists)
        {
            ...
            foreach (var item in metricType)
            {
                ...
                while (count <= numberOfCharts)
                {
                    ...
                    var genderData = slideFourData.Where(x => x.MetricType.Contains(item) && x.SelectionName == selectionName).ToList();
        ...
        private void BindSlideData(List<PPTBindingData> pptData, ISlide cur_Slide, int numberOfCharts, int SlideNumber, List<ImagesList> imagesLists)
        {
            ...
            for (int i = 7; i >= 1; i--)
            {
                ...
                var obj = selectionList.Find(a => a.SelectionName == benchamrkData[index].SelectionName);
            ...
            for (int i = 2; i <= selectionList.Count; i++)
            {
                ...
                {
                    var dataList = comparisonData.Where(x => x.SelectionName == item.SelectionName).ToList();
                    ...
                    for (int j = 7; j >= 0; j--)
                    {
                        if (j == 0)
                        {
                            ...
                            var obj = selectionList.Find(a => a.SelectionName == item.SelectionName);
            ...
            for (int i = 0; i <= 7; i++)
            {
                if (i == 0)
                {              
                    var obj = selectionList.Find(a => a.SelectionName == benchamrkDataDaypart[index].SelectionName);
            ...
            for (int i = 2; i <= selectionList.Count; i++)
            {
                ...
                if (item.SelectionType == "Comparison")
                {
                    var dataList = comparisonDataDaypart.Where(x => x.SelectionName == item.SelectionName).ToList();
        ...
        private void BindSlideSixSevenAndTenData(List<PPTBindingData> pptData, ISlide cur_Slide, int numberOfCharts, int SlideNumber, List<ImagesList> imagesLists)
        {
            ...
            maxValue = maxValue<0.9? maxValue + 0.1:maxValue;
        ...
        private void BindSlideNineData(List<PPTBindingData> pptData, ISlide cur_Slide, int numberOfCharts, int SlideNumber, List<ImagesList> imagesLists)
        {
            ...
            foreach (var item in selectionList)
            {
                ...
                else
                {
                    ...
                    var compareDataLL = whereLL.Where(x => x.SelectionType == "Comparison" && x.SelectionName == item.SelectionName).ToList();
                    var compareDataHL = whereHL.Where(x => x.SelectionType == "Comparison" && x.SelectionName == item.SelectionName).ToList();
                    for (int j = 1; j < 2; j++)
                    {
                        for (int k = 0; k <= 1; k++)
                        {
        ...
        private void BindSlideElevenData(List<PPTBindingData> pptData, ISlide cur_Slide, int numberOfCharts, int SlideNumber, List<ImagesList> imagesLists)
        {
            ...
            foreach (var item in selectionList)
            {
                ...
                {
                    index = 0;
                    var compareData = slideData.Where(x => x.SelectionType == "Comparison" && x.SelectionName == item.SelectionName).ToList();
                    for (int i = 1; i <= 10; i++)
                    {
                        if (i != 7)
                        {
        ...
        private void BindSlideFourteenData(List<PPTBindingData> pptData, ISlide cur_Slide, int numberOfCharts, int SlideNumber, List<ImagesList> imagesLists)
        {
            int index = 0;
            int chartCount = 0;
            numberOfCharts++;
            ...
            foreach (var item in selectionList)
            {
                ...
                {
                    var compareData = slideFourteenData.Where(x => x.SelectionType == "Comparison" && x.SelectionName == item.SelectionName).ToList();
                    for (int i = 1; i <= categoryCount; i++)
                    {
                        metricPercentage = 0.0;
